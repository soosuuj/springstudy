 ...
 * 2. 메소드이름과 호출할 쿼리문의 아이디(id)를 동일하게 맞추면 자동으로 호출된다.  -이미함
 * 3. @Mapper로 등록된 인터페이스의 검색이 가능하도록 @MapperScan을 추가해야 한다.  -appconfig
 *    (SqlSessionTemplate Bean이  등록된 AppConfig.java에 추가한다.) 
 * 4. 매퍼의 namespace 값을 인터페이스 경로로 작성한다. 
 */

@Mapper // 1번 - @Mapper, void 빼고 interface로 변경

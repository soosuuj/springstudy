...
--    첨부가 없는 게시글도 초회할 수 있도록 외부 조인을 사용함
--    작성자가 없는 업로드 게시글도 조회할 수 있도록 외부 조인을 사용함
SELECT UP.UPLOAD_NO, UP.TITLE, UP.CONTENTS, UP.CREATED_AT, UP.MODIFIED_AT, ATC.PATH, ATC.ORIGINAL_FILENAME, ATC.FILESYSTEM_NAME, ATC.DOWNLOAD_COUNT, ATC.HAS_THUMBNAIL, COUNT(ATC.ATTACH_NO) AS 첨부파일수, USR.USER_NO, USR.EMAIL, USR.NAME
 ...
 GROUP BY UP.UPLOAD_NO, UP.TITLE, UP.CONTENTS, UP.CREATED_AT, UP.MODIFIED_AT, ATC.PATH, ATC.ORIGINAL_FILENAME, ATC.FILESYSTEM_NAME, ATC.DOWNLOAD_COUNT, ATC.HAS_THUMBNAIL, USR.USER_NO, USR.EMAIL, USR.NAME; 

-- 첨부 테이블 정보를 조회하지 않는다면 업로드 게시글 - 첨부의 조인을 제거하고 작업을 수행할 수 있음
SELECT A.UPLOAD_NO, A.TITLE, A.CONTENTS, A.CREATED_AT, A.MODIFIED_AT, A.ATTCH_COUNT, A.USER_NO, A.EMAIL, A.NAME
  FROM (SELECT ROW_NUMBER() OVER(ORDER BY UPLOAD_NO DESC) AS RN,
        UP.UPLOAD_NO, UP.TITLE, UP.CONTENTS, UP.CREATED_AT, UP.MODIFIED_AT,
          (SELECT COUNT(*) FROM ATTACH_T ATC WHERE UP.UPLOAD_NO = ATC.UPLOAD_NO) AS ATTCH_COUNT, 
          USR.USER_NO, USR.EMAIL, USR.NAME
             FROM UPLOAD_T UP LEFT OUTER JOIN USER_T USR
               ON UP.USER_NO = USR.USER_NO) A
    WHERE A.RN BETWEEN 4 AND 6;
